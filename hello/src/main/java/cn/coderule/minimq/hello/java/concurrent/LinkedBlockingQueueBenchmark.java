package cn.coderule.minimq.hello.java.concurrent;
...
public final class LinkedBlockingQueueBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("LinkedBlockingQueue benchmark");
        ...
        runSingleThread(warmupRounds, measureRounds, opsPerProducer, capacity);
        System.out.println();
        runMpmc(warmupRounds, measureRounds, producers, consumers, opsPerProducer, capacity);
    }

    private static void runSingleThread(int warmupRounds, int measureRounds, int ops, int capacity) {
        for (int i = 0; i < warmupRounds; i++) {
            singleThreadOnce(ops, capacity);
        }

        long best = Long.MAX_VALUE;
        for (int i = 0; i < measureRounds; i++) {
            long nanos = singleThreadOnce(ops, capacity);
    ...
    private static long singleThreadOnce(int ops, int capacity) {
    ...
    private static void runMpmc(int warmupRounds, int measureRounds,
                               int producers, int consumers, int opsPerProducer, int capacity) throws Exception {
        for (int i = 0; i < warmupRounds; i++) {
            mpmcOnce(producers, consumers, opsPerProducer, capacity, true);
        }

        long best = Long.MAX_VALUE;
        long totalOps = (long) producers * opsPerProducer;
        for (int i = 0; i < measureRounds; i++) {
            long nanos = mpmcOnce(producers, consumers, opsPerProducer, capacity, false);
            best = Math.min(best, nanos);
    ...
    private static long mpmcOnce(int producers, int consumers, int opsPerProducer, int capacity, boolean warmup) throws Exception {
        LinkedBlockingQueue<Integer> queue = new LinkedBlockingQueue<>(capacity);
